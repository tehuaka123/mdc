...
int64 nChainStartTime = 1463268757;
...
const string strMessageMagic = "ManderCoin Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // 2014-04-19 Adriano https://ManderCointalk.org/index.php?action=profile;u=112568
        // The following address was lost during distribution with 196780608.602771 coins in it. Blocking just in case :-)
        static const CManderCoinAddress lostWallet ("CKGK6MFmBkreG7k5sU8gDEJNVJ57QZtN3H");
        uint256 hashBlock;
        CTransaction txPrev;
        ...
        if(GetTransaction(txin.prevout.hash, txPrev, hashBlock)){  // get the vin's previous transaction
            ...
            if (ExtractDestination(txPrev.vout[txin.prevout.n].scriptPubKey, source)){  // extract the destination of the previous transaction's vout[n]
                CManderCoinAddress addressSource(source);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal ManderCoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ManderCoin
    bool fStrictPayToScriptHash = true; // Always active in ManderCoin
    ...
    // ppcoin: fees are not collected by miners as in ManderCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ManderCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "block spacing he first block the with ha ManderCoin";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = nChainStartTime + 88;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 609790 : 609790;
		...
		block.print();
		assert(block.hashMerkleRoot == uint256("0x469467762ae2243571164a4a5792e67a8f33e977e727ae9c6f3d98600dcd9da0"));
...
unsigned char pchMessageStart[4] = { 0xcd, 0xd2, 0xed, 0xbf };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("ManderCoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("ManderCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ManderCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("ManderCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadManderCoinMiner(void* parg);

static bool fGenerateManderCoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void ManderCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("ManderCoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateManderCoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateManderCoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running ManderCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadManderCoinMiner(void* parg)
{
    ...
    try
    {
        ...
        ManderCoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadManderCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadManderCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadManderCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateManderCoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateManderCoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateManderCoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d ManderCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadManderCoinMiner, pwallet))
                printf("Error: NewThread(ThreadManderCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
